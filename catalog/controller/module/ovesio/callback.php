...
class Callback extends \Opencart\System\Engine\Controller
{
    ...
    protected function handle($data)
    {
        ...
        $this->model_extension_ovesio_module_ovesio->addList([
    ...
    protected function generateDescriptionProduct($product_id, $data)
    {
        ...
        $this->model_extension_ovesio_module_ovesio->updateProductDescription($product_id, $data['language_id'], $product_description);
    ...
    protected function generateDescriptionCategory($category_id, $data)
    {
        ...
        $this->model_extension_ovesio_module_ovesio->updateCategoryDescription($category_id, $data['language_id'], $category_description);
    ...
    protected function metatagsProduct($product_id, $data)
    {
        ...
        $seo = $this->model_extension_ovesio_module_ovesio->getProductForSeo($product_id, $language_id);
        ...
        $this->model_extension_ovesio_module_ovesio->updateProductDescription($product_id, $language_id, $metatags);
    }
    ...
    protected function metatagsCategory($category_id, $data)
    {
        ...
        $language_id = $data['language_id'];
        $seo = $this->model_extension_ovesio_module_ovesio->getCategoryForSeo($category_id, $language_id);
        $content = $this->populateCompatibilityContent($data['content']);
        ...
        $this->model_extension_ovesio_module_ovesio->updateCategoryDescription($category_id, $language_id, $metatags);
    }
    ...
    protected function translateProduct($product_id, $data)
    {
        ...
        foreach ($attribute_values as $attribute_id => $text) {
            $this->model_extension_ovesio_module_ovesio->updateAttributeValueDescription($product_id, $attribute_id, $data['language_id'], $text);
        }
    ...
    protected function translateAttributeGroup($attribute_group_id, $data)
    {
        foreach ($data['content'] as $item) {
            if (strpos($item['key'], 'ag-') === 0) {
                ...
                $this->model_extension_ovesio_module_ovesio->updateAttributeGroupDescription($attribute_group_id, $data['language_id'], $item['value']);
            } elseif (strpos($item['key'], 'a-') === 0) {
                $attribute_id = str_replace('a-', '', $item['key']);
                $this->model_extension_ovesio_module_ovesio->updateAttributeDescription($attribute_id, $data['language_id'], $item['value']);
            } else {
                $this->output['warnings'][] = 'Unknown key "' . $item['key'] . '"';
            }
        }
    ...
    protected function translateOption($option_id, $data)
    {
        foreach ($data['content'] as $item) {
            if (strpos($item['key'], 'o-') === 0) {
                ...
                $this->model_extension_ovesio_module_ovesio->updateOptionDescription($option_id, $data['language_id'], $item['value']);
            } elseif (strpos($item['key'], 'ov-') === 0) {
                $option_value_id = str_replace('ov-', '', $item['key']);
                $this->model_extension_ovesio_module_ovesio->updateOptionValueDescription($option_value_id, $data['language_id'], $item['value']);
            } else {
                $this->output['warnings'][] = 'Unknown key "' . $item['key'] . '"';
            }
        }
    ...
    private function seoProduct($product_id, $language_id, $product_description)
    {
        ...
        $data = $this->model_extension_ovesio_module_ovesio->getProductForSeo($product_id, $language_id);
        ...
        // OC4 Events are better, but we don't know the exact controller path for Complete SEO in OC4.
        // Assuming user fixes compatibility with Complete SEO if they have it.
    ...
    private function seoCategory($category_id, $language_id, $category_description)
    {
        ...
        $data = $this->model_extension_ovesio_module_ovesio->getCategoryForSeo($category_id, $language_id);
    ...
    public function updateActivityStatus()
    {
        ...
        $activity = $this->model_extension_ovesio_module_ovesio->getActivityById($activity_id);
