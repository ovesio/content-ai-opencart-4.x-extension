...
// Autoloader should be handled differently or strictly require path
require_once(DIR_EXTENSION . 'ovesio/system/library/ovesio/sdk/autoload.php');

class Ovesio extends \Opencart\System\Engine\Controller
{
    private $token = 'user_token';
    private $module_key = 'module_ovesio';
    // OC 4 uses 'event' table/model but via 'extension/event' is fine or 'setting/event'
    private $event_model = 'setting/event';
	...
	public function index(): void {
        ...
        $data['connect_form']          = $this->view('extension/ovesio/module/ovesio_connect_form', $data);
        ...
        $data['count_errors']    = $this->model_extension_ovesio_module_ovesio->getActivitiesTotal(['status' => 'error']);
        $data['url_list_errors'] = $this->url->link('extension/ovesio/module/ovesio.activityList', $this->tokenQs() . '&status=error');

		$data['header']      = $this->load->controller('common/header');
		$data['column_left'] = $this->load->controller('common/column_left');
		$data['footer']      = $this->load->controller('common/footer');

		$this->response->setOutput($this->view('extension/ovesio/module/ovesio', $data));
	}
    ...
    public function generateContentCard($return)
    {
        ...
        $data['url_edit'] = $this->url->link('extension/ovesio/module/ovesio.generateContentForm', $this->tokenQs());
        ...
        $html = $this->view('extension/ovesio/module/ovesio_generate_content_card', $data);
    ...
    public function generateContentForm()
    {
        ...
        $data['action'] = $this->url->link('extension/ovesio/module/ovesio.generateContentFormSave', $this->tokenQs());

        if ($data['workflows_list']) {
            $html = $this->view('extension/ovesio/module/ovesio_generate_content_form', $data);
        } else {
    ...
    public function generateSeoCard($return)
    {
        ...
        $data['url_edit'] = $this->url->link('extension/ovesio/module/ovesio.generateSeoForm', $this->tokenQs());
        ...
        $html = $this->view('extension/ovesio/module/ovesio_generate_seo_card', $data);
    ...
    public function generateSeoForm()
    {
        ...
        $data['action'] = $this->url->link('extension/ovesio/module/ovesio.generateSeoFormSave', $this->tokenQs());

        if ($data['workflows_list']) {
            $html = $this->view('extension/ovesio/module/ovesio_generate_seo_form', $data);
        } else {
            $html = $this->language->get('text_api_error');
        }
    ...
    public function translateCard($return)
    {
        ...
        $data['url_edit'] = $this->url->link('extension/ovesio/module/ovesio.translateForm', $this->tokenQs());
        ...
        $html = $this->view('extension/ovesio/module/ovesio_translate_card', $data);
    ...
    public function translateForm()
    {
        ...
        $data['action'] = $this->url->link('extension/ovesio/module/ovesio.translateFormSave', $this->tokenQs());

        if ($data['workflows_list'] && $data['ovesio_languages']) {
            $html = $this->view('extension/ovesio/module/ovesio_translate_form', $data);
        } else {
            $html = $this->language->get('text_api_error');
        }
